...
export default function TextArea(
  ...
  const [local, rest] = splitProps(props, ["label", "hint", "class", "error"]);
  return (
    <label class={cx("flex flex-col gap-1.5 text-sm", local.class)}>
      ...
      <textarea
        class={cx(
          "min-h-24 resize-y rounded-xl border bg-surface px-3 py-2 text-ink outline-none transition placeholder:text-ink-subtle focus:border-accent",
