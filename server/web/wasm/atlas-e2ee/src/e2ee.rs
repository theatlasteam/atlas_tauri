...
fn store_identity(secret: &StaticSecret) -> Result<(), E2eeError> {
    crate::store::secret_set(IDENTITY_SECRET_KEY, B64.encode(secret.to_bytes()))
        .map_err(E2eeError::Storage)
}

fn identity() -> Result<StaticSecret, E2eeError> {
    let _guard = IDENTITY_LOCK.lock().unwrap();

    if let Some(b64) = crate::store::secret_get(IDENTITY_SECRET_KEY).map_err(E2eeError::Storage)? {
...
pub fn e2ee_public_key() -> Result<String, E2eeError> {
    let secret = identity()?;
...
pub fn e2ee_seal(peer_public_key: String, plaintext: String) -> Result<String, E2eeError> {
    let secret = identity()?;
    let peer = parse_public(&peer_public_key)?;
    seal(&secret, &peer, &plaintext)
}

pub fn e2ee_open(peer_public_key: String, body: String) -> Result<String, E2eeError> {
    let secret = identity()?;
    let peer = parse_public(&peer_public_key)?;
