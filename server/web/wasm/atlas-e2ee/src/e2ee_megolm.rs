//! Browser Megolm — same wire format as src-tauri/src/e2ee_megolm.rs.
...
fn take_out(chat: &str) -> Result<Option<GroupSession>, E2eeError> {
    ...
    drop(cache);
    let key = pickle_key()?;
    if let Some(p) = crate::store::secret_get(out_key(chat)).map_err(E2eeError::Storage)? {
...
fn put_out(chat: &str, session: GroupSession) -> Result<(), E2eeError> {
    let key = pickle_key()?;
    let pickled = session.pickle().encrypt(&key);
    crate::store::secret_set(out_key(chat), pickled).map_err(E2eeError::Storage)?;
    OUT.lock()
        .unwrap()
...
fn take_in(k: &str) -> Result<Option<InboundGroupSession>, E2eeError> {
    ...
    drop(cache);
    let key = pickle_key()?;
    if let Some(p) = crate::store::secret_get(k).map_err(E2eeError::Storage)? {
...
fn put_in(k: &str, session: InboundGroupSession) -> Result<(), E2eeError> {
    let key = pickle_key()?;
    let pickled = session.pickle().encrypt(&key);
    crate::store::secret_set(k, pickled).map_err(E2eeError::Storage)?;
    IN.lock()
        .unwrap()
        .get_or_insert_with(HashMap::new)
...
pub fn megolm_encrypt(chat_id: String, plaintext: String) -> Result<MegolmSeal, E2eeError> {
    let mut session = match take_out(&chat_id)? {
...
pub fn megolm_import_key(
    ...
    if let Some(existing) = take_in(&k)? {
        put_in(&k, existing)?;
        return Ok(());
    }
...
pub fn megolm_decrypt(
    ...
    let k = in_key(&chat_id, &sender_id, &session_id);
    let mut session = take_in(&k)?.ok_or_else(|| E2eeError::Bad("no megolm session".into()))?;
